...
import {
  CreditCard, Trash2, Star, Plus, ShieldCheck, X,
} from "lucide-react";
...
interface PaymentMethod {
  id: string; type: string; lastFour: string; isDefault: boolean;
}
...
const pmApi = {
  ...
  createSetupIntent:() => $api.post<{ clientSecret: string }>("/payment-methods/setup-intent"),
...
export default function Payments() {
  ...
  const load = () => {
    pmApi.getAll()
      .then((r) => { setMethods(r.data); setLoading(false); })
  ...
  return (
    <div className="py-8 max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        {!showAdd && (
          <AppButton icon={Plus} onClick={handleAddClick} className="rounded-xl px-4 py-2.5 text-sm">
            Add card
          </AppButton>
        )}
      </div>
      ...
      {showAdd && clientSecret && (
        <Elements stripe={stripePromise} options={{ clientSecret }}>
          <AddCardForm
            ...
            onSaved={handleSaved}
            onCancel={() => { setShowAdd(false); setClientSecret(""); }}
          />
        </Elements>
      )}
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2].map((i) => <div key={i} className="h-20 bg-surface rounded-2xl animate-pulse border border-[#D7E2EE]" />)}
        </div>
      ) : methods.length === 0 ? (
        <div className="py-16 flex flex-col items-center gap-3 text-center">
          <CreditCard size={48} strokeWidth={1} className="text-foreground-muted" />
          <p className="font-semibold">No payment methods saved</p>
          <p className="text-sm text-foreground-muted">Add a card to speed up your future bookings</p>
        </div>
      ) : (
        <div className="space-y-3">
          {methods.map((m) => (
      ...
      <div className="mt-8 flex items-start gap-3 p-4 bg-primary-muted rounded-xl text-sm text-foreground-muted">
        <ShieldCheck size={18} strokeWidth={1.5} className="text-primary shrink-0 mt-0.5" />
        <p>Your card details are securely stored by Stripe and never touch our servers. We comply with PCI DSS standards.</p>
      </div>
    </div>
  );
}
...
function CardRow({ method, onSetDefault, onRemove }: { method: PaymentMethod; onSetDefault: () => void; onRemove: () => void }) {
  ...
  return (
    <div className={cn(
      ...
      method.isDefault ? "border-primary bg-primary-muted" : "border-[#D7E2EE] bg-surface hover:border-primary/50",
    )}>
      {/* Brand badge */}
      <div className={cn(
...
function AddCardForm({ clientSecret, onSaved, onCancel }: { clientSecret: string; onSaved: (m: any) => void; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface rounded-2xl border border-[#D7E2EE] p-5 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold">Add new card</h3>
        <button type="button" onClick={onCancel} className="p-1 rounded-lg hover:bg-background text-foreground-muted">
      ...
      <div className="p-3 border-2 border-input-secondary rounded-lg hover:border-primary focus-within:border-primary transition-colors mb-4">
        <CardElement
          options={{
            style: {
              base: { fontSize: "15px", color: "#1c1b1f", "::placeholder": { color: "#79747E" } },
      ...
      <div className="flex gap-3">
        <AppButton type="submit" disabled={!stripe || saving} className="rounded-xl px-5">
          {saving ? "Saving…" : "Save card"}
        </AppButton>
        <AppButton type="button" intent="outline" onClick={onCancel} className="rounded-xl px-5">
          Cancel
        </AppButton>
      </div>

      <p className="text-xs text-foreground-muted mt-3 flex items-center gap-1.5">
        <ShieldCheck size={12} strokeWidth={1.5} className="text-primary" />
