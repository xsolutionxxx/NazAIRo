...
export default function FlightList() {
  ...
  useEffect(() => {
    ...
    const params: IFlightSearchParams = {
      ...
      departureTimeFrom: searchParams.get("departureTimeFrom") ? Number(searchParams.get("departureTimeFrom")) : undefined,
      departureTimeTo: searchParams.get("departureTimeTo") ? Number(searchParams.get("departureTimeTo")) : undefined,
  ...
  if (!searchParams.get("from")) {
    return (
      <EmptyState
        icon={<Plane size={40} strokeWidth={1} className="text-primary" />}
  ...
  if (!flights.length) {
    return (
      <EmptyState
        icon={<Plane size={40} strokeWidth={1} className="text-foreground-muted" />}
  ...
  return (
    <div className="space-y-4">
      ...
      {returnFlights && (
        <p className="text-sm font-semibold text-primary">Outbound flights</p>
      )}
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="h-36 bg-surface rounded-2xl border border-[#D7E2EE] animate-pulse" />
      ))}
    </div>
  );
}
