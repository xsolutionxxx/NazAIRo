...
export default function FlightSearchForm() {
  ...
  const { airports, airportsLoading } = useAppSelector((s) => s.flightReducer);
  ...
  useEffect(() => {
    ...
    if (sp.get("from")) setFromQuery(sp.get("from")!);
    if (sp.get("to")) setToQuery(sp.get("to")!);
    if (sp.get("date")) setDepartureDate(sp.get("date")!);
    if (sp.get("returnDate")) { setReturnDate(sp.get("returnDate")!); setTripType("round-trip"); }
    if (sp.get("passengers")) setPassengers(Number(sp.get("passengers")));
    if (sp.get("cabinClass")) setCabinClass(sp.get("cabinClass") as CabinClass);
  }, []);
  ...
  return (
    <div className="bg-surface rounded-2xl shadow-sm border border-[#D7E2EE] p-6">
      ...
      <div className="grid grid-cols-12 gap-4 items-end">
        {/* From */}
        <div className="col-span-12 md:col-span-3 relative" ref={fromRef}>
          <label className="block text-xs font-medium text-foreground-muted mb-1.5">From</label>
          <div className="relative">
            <input
              value={fromQuery}
              onChange={(e) => handleFromQuery(e.target.value)}
              onFocus={() => fromQuery.length >= 2 && setFromOpen(true)}
          ...
          {fromOpen && airports.length > 0 && (
            <AirportDropdown
              ...
              onSelect={(a) => { setFrom(a); setFromQuery(`${a.city} (${a.iata})`); setFromOpen(false); }}
            />
          )}
        </div>
        ...
        <div className="hidden md:flex col-span-12 md:col-span-1 justify-center pb-1">
          <button
            onClick={handleSwap}
            className="h-10 w-10 rounded-full border-2 border-input-secondary hover:border-primary flex items-center justify-center transition-all hover:bg-primary-muted"
          >
        ...
        <div className="col-span-12 md:col-span-3 relative" ref={toRef}>
          <label className="block text-xs font-medium text-foreground-muted mb-1.5">To</label>
          <div className="relative">
            <input
              value={toQuery}
              onChange={(e) => handleToQuery(e.target.value)}
              onFocus={() => toQuery.length >= 2 && setToOpen(true)}
              placeholder="City or airport"
              className={cn(
                "w-full h-14 px-4 border-2 border-input-secondary rounded-lg text-sm bg-transparent outline-none transition-all",
                "hover:border-primary focus:border-primary",
          ...
          {toOpen && airports.length > 0 && (
            <AirportDropdown
              airports={airports}
              loading={airportsLoading}
              onSelect={(a) => { setTo(a); setToQuery(`${a.city} (${a.iata})`); setToOpen(false); }}
            />
          )}
        </div>
        ...
        <div className="col-span-12 md:col-span-2">
          ...
          <div className="relative">
            ...
            />
            <CalendarDays size={16} strokeWidth={1.5} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-foreground-muted" />
          </div>
        </div>
        ...
        {tripType === "round-trip" && (
          <div className="col-span-12 md:col-span-2">
            <label className="block text-xs font-medium text-foreground-muted mb-1.5">Return</label>
            <div className="relative">
              <input
                type="date"
        ...
        <div className="col-span-12 md:col-span-2" ref={cabinRef}>
          <label className="block text-xs font-medium text-foreground-muted mb-1.5">Passengers & Class</label>
          <button
            onClick={() => setCabinOpen((p) => !p)}
            className="w-full h-14 px-4 border-2 border-input-secondary rounded-lg text-sm bg-transparent outline-none transition-all hover:border-primary flex items-center justify-between"
          >
            <span>
              {passengers} pax · {CABIN_OPTIONS.find((c) => c.value === cabinClass)?.label}
            </span>
            <ChevronDown size={16} strokeWidth={1.5} className={cn("transition-transform", cabinOpen && "rotate-180")} />
          </button>

          {cabinOpen && (
            <div className="absolute z-50 mt-1 bg-surface rounded-xl shadow-lg border border-[#D7E2EE] p-4 w-72">
              {/* Passengers counter */}
              <p className="text-xs font-semibold text-foreground-muted uppercase tracking-wide mb-3">Passengers</p>
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm">Adults</span>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setPassengers((p) => Math.max(1, p - 1))}
                    className="h-8 w-8 rounded-full border border-input-secondary flex items-center justify-center hover:border-primary transition-all"
                  >
                    <Minus size={14} />
                  </button>
                  <span className="w-4 text-center font-semibold">{passengers}</span>
                  <button
              ...
              {/* Cabin class */}
              <p className="text-xs font-semibold text-foreground-muted uppercase tracking-wide mb-3">Cabin Class</p>
              <div className="grid grid-cols-2 gap-2">
                {CABIN_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={cn(
                      "py-2 px-3 rounded-lg text-xs font-medium border-2 transition-all text-left",
        ...
        {/* Search button */}
        <div className={cn("col-span-12", tripType === "round-trip" ? "md:col-span-1" : "md:col-span-1")}>
          <AppButton
...
function AirportDropdown({
  ...
  airports: { id: string; iata: string; name: string; city: string; country: string }[];
  loading: boolean;
  ...
  return (
    <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-surface rounded-xl shadow-lg border border-[#D7E2EE] overflow-hidden">
      {loading ? (
        <div className="p-3 text-sm text-foreground-muted">Searching...</div>
      ) : (
        airports.map((a) => (
          <button
            key={a.id}
          ...
          >
            <span className="font-mono font-bold text-sm text-primary w-10 shrink-0">{a.iata}</span>
            <div className="min-w-0">
              <p className="text-sm font-medium truncate">{a.city}</p>
              <p className="text-xs text-foreground-muted truncate">{a.name}</p>
            </div>
            <span className="ml-auto text-xs text-foreground-muted shrink-0">{a.country}</span>
          </button>
        ))
      )}
    </div>
