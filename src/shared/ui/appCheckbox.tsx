...
export const AppCheckbox = React.forwardRef<HTMLInputElement, AppCheckboxProps>(({ id, name, label, containerClassName, className, ...props}, ref) => {
    const generatedId = React.useId();
    const checkboxId = id || name || generatedId;

    return (
        <label htmlFor={checkboxId} className={cn("flex items-center gap-2.5  cursor-pointer", containerClassName)}>
            <input
                id={checkboxId}
                type="checkbox"
                ref={ref}
